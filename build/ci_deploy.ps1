...
$Global:flavour_suffix = $Global:flavours.Count -eq 1 ? "_$($Global:flavours[0])" : ''
...
function Deploy-NightlyFiles {
    ...
    # upload zip
    $path_zip_local  = "$PSScriptRoot/../~output/xvm_${vcs_tag}_${vcs_commits}_${vcs_branch}_${vcs_hash}$($Global:flavour_suffix).zip"
    $path_zip_remote = "$Global:ssh_nightly_path/${vcs_branch}/"
    Invoke-SftpUpload -LocalPath $path_zip_local -RemotePath $path_zip_remote
        
    # upload installer
    $path_installer_local   = "$PSScriptRoot/../~output/xvm_${vcs_tag}_${vcs_commits}_${vcs_branch}_${vcs_hash}$($Global:flavour_suffix).exe"
...
function Deploy-NightlyMeta {
    ...
    $versionWg = 'wg' -in $Global:flavours ? $build_info.game_version_wg : ''
    $versionLesta = 'lesta' -in $Global:flavours ? $build_info.game_version_lesta : ''

    # MODXVM meta
    $mxDate = Get-Date -Format "yyyy-MM-ddTHH:mm:sszzz"
    $mxSizeExe = (Get-Item -Path "$PSScriptRoot/../~output/xvm_${vcs_tag}_${vcs_commits}_${vcs_branch}_${vcs_hash}$($Global:flavour_suffix).exe").Length
    $mxSizeZip = (Get-Item -Path "$PSScriptRoot/../~output/xvm_${vcs_tag}_${vcs_commits}_${vcs_branch}_${vcs_hash}$($Global:flavour_suffix).zip").Length
    $mxUrlZip = "https://nightly.modxvm.com/download/${vcs_branch}/xvm_${vcs_tag}_${vcs_commits}_${vcs_branch}_${vcs_hash}$($Global:flavour_suffix).zip"
    $mxUrlExe = "https://nightly.modxvm.com/download/${vcs_branch}/xvm_${vcs_tag}_${vcs_commits}_${vcs_branch}_${vcs_hash}$($Global:flavour_suffix).exe"

    $path_meta_template    = "$PSScriptRoot/ci_deploy.json.template"
    $path_meta_local       = "$PSScriptRoot/../~output/dl_meta$($Global:flavour_suffix).json"
    ...
    $template -replace 'XVMBUILD_MX_DATE', $mxDate `
        ...
        -replace 'XVMBUILD_WOT_VERSION_wg', $versionWg `
        -replace 'XVMBUILD_WOT_VERSION_lesta', $versionLesta `
