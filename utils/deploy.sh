...
install_xfw()
{
  ...
  cp -a ../~output/$XVMBUILD_FLAVOR/wotmod/com.modxvm.*.wotmod "$WOT_PATH/mods/$XVMBUILD_WOT_VERSION/" || err "[install_xfw]"
  # for debug only
  mkdir -p "$WOT_PATH/res_mods/$XVMBUILD_WOT_VERSION/gui/flash/" || err "[install_xfw]"
  cp -a ../~output/$XVMBUILD_FLAVOR/swf/*.swf "$WOT_PATH/res_mods/$XVMBUILD_WOT_VERSION/gui/flash/" || err "[install_xfw]"
}

install_xvm()
{
  echo "=> xvm"
  rm -rf "$WOT_PATH/res_mods/mods/openwg_packages" || err "[install_xvm] rm"
  mkdir -p "$WOT_PATH/res_mods/mods/openwg_packages" || err "[install_xvm] mkdir"
  cp -a ../~output/$XVMBUILD_FLAVOR/xvm/res_mods/mods/openwg_packages/xvm_* "$WOT_PATH/res_mods/mods/openwg_packages/" || err "[install_xvm] cp"
}
