...
def load_file(fn):
    try:
        ...
        if fn != openwg_loader.PATH_ROOT + 'res_mods/configs/xvm/xvm.xc':
            logtrace(__file__)
        return None
...
def resolve_path(path, basepath=None):
    ...
    if path[:6].lower() == "res://":
        path = path.replace("res://", openwg_loader.PATH_ROOT + "res_mods/mods/shared_resources/", 1)
    elif path[:6].lower() == "xvm://":
        path = path.replace("xvm://", openwg_loader.PATH_ROOT + "res_mods/mods/shared_resources/xvm/", 1)
    elif path[:6].lower() == "cfg://":
        path = path.replace("cfg://", openwg_loader.PATH_ROOT + "res_mods/configs/xvm/", 1)
    elif basepath:
