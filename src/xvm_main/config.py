...
def load(e, from_init_stage = False):
    global config_autoreload
    global config_data
    global lang_data

    trace('xvm_main.config::load()')
    try:
    ...
    trace('xvm_main.config::load() --> end')
    g_eventBus.handleEvent(events.HasCtxEvent(XVM_EVENT.CONFIG_LOADED, {'fromInitStage': from_init_stage}))
...
def _tuneup_config(config):
    ...
    config['__xvmRevision'] =  XVM.XVM_REVISION
    config['__wotVersion'] = openwg_loader.WOT_VERSION_SHORT
