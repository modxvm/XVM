...
package com.xvm.battle.playersPanel
{
    ...
    import net.wg.gui.battle.epicRandom.views.stats.components.playersPanel.list.*;
    import net.wg.gui.battle.views.stats.constants.*;
    import net.wg.infrastructure.interfaces.*;
    import scaleform.clik.core.*;

    public class EpicRandomPlayersPanelListItemProxy extends UIComponent implements IExtraFieldGroupHolder
    {
        ...
        public function EpicRandomPlayersPanelListItemProxy(ui:PlayersPanelListItem, isLeftPanel:Boolean)
        {
            this.ui = ui;
            mouseEnabled = false;
            mouseChildren = false;
        ...
        public function setVehicleIcon(vehicleImage:String):void
        {
            ...
            if (mcfg == null)
                return;
            var atlasName:String = isLeftPanel ? UI_EpicRandomPlayersPanel.playersPanelLeftAtlas : UI_EpicRandomPlayersPanel.playersPanelRightAtlas;
        ...
        {
            return UI_EpicRandomPlayersPanel.fix_state(ui.xfw_state);
        }
        ...
        private function setup():void
        {
            try
            {
                //Logger.add("EpicRandomPlayersPanelListItemProxy.onConfigLoaded()");
                bcfg = Config.config.battle;
                pcfg = Config.config.playersPanel;
                mcfg = pcfg[UI_EpicRandomPlayersPanel.PLAYERS_PANEL_STATE_NAMES[(state == -1 || state == PLAYERS_PANEL_STATE.HIDEN) ? PLAYERS_PANEL_STATE.LONG : state]];
                ncfg = pcfg.none;
        ...
        private function applyState():void
        {
            ...
            switch (state)
            {
                ...
                case PLAYERS_PANEL_STATE.SHORT:
                    mcfg = pcfg[UI_EpicRandomPlayersPanel.PLAYERS_PANEL_STATE_NAMES[state]];
                    mopt_removeSquadIcon = Macros.FormatBooleanGlobal(mcfg.removeSquadIcon);
        ...
        private function createExtraFields():void
        {
            ...
            if (cfg.formats && cfg.formats.length)
            {
                ...
                extraFieldsHidden.addEventListener(MouseEvent.MOUSE_MOVE, (BattleXvmView.battlePage.playersPanel as UI_EpicRandomPlayersPanel).onMouseMoveHandler);
                extraFieldsHidden.addEventListener(MouseEvent.ROLL_OVER, (BattleXvmView.battlePage.playersPanel as UI_EpicRandomPlayersPanel).onMouseRollOverHandler);
                extraFieldsHidden.addEventListener(MouseEvent.ROLL_OUT, (BattleXvmView.battlePage.playersPanel as UI_EpicRandomPlayersPanel).onMouseRollOutHandler);
