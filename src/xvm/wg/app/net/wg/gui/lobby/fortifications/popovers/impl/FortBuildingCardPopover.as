package net.wg.gui.lobby.fortifications.popovers.impl
{
   ...
   public class FortBuildingCardPopover extends FortBuildingCardPopoverMeta implements IFortBuildingCardPopoverMeta
   {
      ...
      override protected function setData(param1:BuildingCardPopoverVO) : void {
         ...
         this.assignPlayers.setData(this.model.assignLbl,this.model.playerCount,this.model.isTutorial);
         if(!_loc2_)
         {
      ...
      private function invalidateFocus() : void {
         ...
         {
            if((this.model.buildingHeader.isModernization) && (this.header.upgradeBtn.visible))
            {
               setFocus(this.header.upgradeBtn);
            }
            else
            {
               if(this.controlPanel.currentState == FortPopoverControlPanel.ACTION_STATE && (this.controlPanel.visible) && (this.controlPanel.actionButton.enabled))
               {
