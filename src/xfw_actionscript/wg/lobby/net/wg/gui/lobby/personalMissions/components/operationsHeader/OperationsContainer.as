    ...
    public class OperationsContainer extends UIComponentEx
    {
        ...
        public function layoutContent() : void
        {
            ...
            while(_loc5_ < _loc1_)
            {
                ...
                _loc4_ = _loc4_ + _loc2_.width;
                if(_loc5_ < _loc1_ - 1)
                {
                    _loc3_ = this._separators[_loc5_];
                    _loc3_.y = SEPARATOR_TOP;
                    _loc4_ = _loc4_ + (_loc2_.selected?0:SEPARATOR_GAP_LEFT);
                    this.playTween(_loc3_,_loc4_,DEF_TWEEN_DURATION,this._firstShow);
                    _loc4_ = _loc4_ + (_loc3_.width + (_loc2_.selected?0:SEPARATOR_GAP_RIGHT));
                }
                _loc5_++;
            }
        ...
        private function updateContent() : void
        {
            ...
            while(_loc5_ < _loc1_)
            {
                ...
                if(_loc5_ > 0)
                {
                    ...
                    _loc3_.alpha = _loc4_.isLocked()?SEPARATOR_ALPHA_LOCKED:SEPARATOR_ALPHA_DEF;
                }
        ...
        {
            var _loc4_:Operation = null;
            var _loc3_:Vector.<Operation> = this._operations.splice(param1,param2);
            for each(_loc4_ in _loc3_)
            {
                _loc4_.removeEventListener(OperationEvent.CLICK,this.onOperationClickHandler);
                _loc4_.dispose();
                removeChild(_loc4_);
            }
        }
