package net.wg.gui.lobby.window
{
    ...
    public class MissionAwardWindow extends MissionAwardWindowMeta implements IMissionAwardWindowMeta
    {
        ...
        private function updateAwards() : void
        {
            ...
            var _loc3_:* = (WINDOW_WIDTH - _loc1_ * AWARD_RENDERER_WIDTH) / (_loc1_ - 1) >> 1;
            var _loc4_:* = AWARD_RENDERER_WIDTH >> 1;
            if(_loc3_ > _loc4_)
            {
                _loc3_ = _loc4_;
            }
                ...
                if(this._dataVO.availableNextQuest)
                {
                    ...
                    while(_loc3_ < QUEST_CONDITION_COUNTS)
                    {
                        ...
                        if(_loc2_.visible)
                        {
                            _loc2_.y = _loc1_;
                            _loc1_ = _loc1_ + _loc2_.height;
                            this.nextQuestButton.y = this.nextQuestButton.y + _loc2_.height;
                        }
                        _loc3_++;
                    }
                }
