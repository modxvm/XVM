    ...
    public class WTEventEntryPointTextInfo extends MovieClip implements IDisposable
    {
        ...
        public function updateSize(param1:Boolean, param2:Boolean, param3:Number, param4:Number, param5:Boolean = false) : void
        {
            ...
            if(param5 || this._isSmall != param1 || this._isWide != param2)
            {
                ...
                _loc6_ = _loc6_ + (this._isWide?FRAME_SIZE_PREF_WIDE:Values.EMPTY_STR);
                gotoAndStop(_loc6_);
                this.updateTitle();
                this.updateEndTextAndLayout();
            }
        ...
        {
            this.titleTf.text = this._isSmall && !this._isWide?WT_EVENT.WTEVENTSENTRYWIDGETVIEW_TITLE_SMALL:WT_EVENT.WTEVENTSENTRYWIDGETVIEW_TITLE;
        }

        private function updateEndTextAndLayout() : void
        {
            this.timeTf.text = this._endDate;
            this.timeTf.y = this._isSmall?TIME_TF_Y_SMALL:TIME_TF_Y_BIG;
            this.timeIcon.y = this._isSmall?ICON_Y_SMALL:ICON_Y_BIG;
            if(!this._isWide)
            {
                this.timeTf.y = this.timeTf.y + (this._isSmall?SMALL_TIME_Y_OFFSET:BIG_TIME_Y_OFFSET);
                this.timeIcon.y = this.timeIcon.y + (this._isSmall?SMALL_TIMER_Y_OFFSET:BIG_TIMER_Y_OFFSET);
            }
