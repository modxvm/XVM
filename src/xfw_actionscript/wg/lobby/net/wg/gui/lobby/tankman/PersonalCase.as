            ...
            if(_loc6_ && _loc6_.length > 0)
            {
                ...
                while(_loc10_ < _loc7_)
                {
                    ...
                    if(_loc8_.val != 0)
                    {
                        ...
                        {
                            this.modifiersHeaders.htmlText = this.modifiersHeaders.htmlText + BR_TAG;
                        }
                        this.modifiersValues.htmlText = this.modifiersValues.htmlText + ("<font color=\"" + _loc5_ + "\">" + _loc3_ + _loc8_.val + "%</font>" + BR_TAG);
                        this.modifiersNames.htmlText = this.modifiersNames.htmlText + (_loc1_.makeString(MENU.tankmanpersonalcase_modifiers(_loc8_.id)) + BR_TAG);
                        while(this.modifiersNames.numLines > this.modifiersValues.numLines)
                        {
                            this.modifiersHeaders.htmlText = this.modifiersHeaders.htmlText + BR_TAG;
                            this.modifiersValues.htmlText = this.modifiersValues.htmlText + BR_TAG;
                        }
                    }
                    _loc10_++;
                }
