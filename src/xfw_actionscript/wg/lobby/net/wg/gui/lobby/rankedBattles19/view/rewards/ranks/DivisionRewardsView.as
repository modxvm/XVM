package net.wg.gui.lobby.rankedBattles19.view.rewards.ranks
{
    ...
    public class DivisionRewardsView extends UIComponentEx implements IDraggable, IStageSizeDependComponent
    {
        ...
        override protected function draw() : void
        {
            ...
            {
                this._containerGlobalLeftShift = this._viewPadding?this._viewPadding.left >> 1:Values.ZERO;
                invalidateSize();
            }
            if(this._rewards)
            {
        ...
        private function updateComponentsPosition() : void
        {
            ...
            var _loc2_:* = _width > this._containerWidth;
            var _loc3_:* = _width >> 1;
            var _loc4_:* = this._containerWidth >> 1;
            var _loc5_:int = this._containerHeightID == CONTAINER_HEIGHT_ID_SHORT?Values.ZERO:CONTAINER_TOP_PADDING_HIGH;
            if(_loc2_)
            {
        ...
        private function set scrollPosition(param1:Number) : void
        {
            ...
            if(_loc2_ > 0)
            {
                var param1:Number = Math.max(-_loc2_,Math.min(_loc2_,param1));
            }
            else
            {
                param1 = 0;
