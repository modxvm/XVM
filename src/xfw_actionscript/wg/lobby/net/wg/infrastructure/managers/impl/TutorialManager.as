package net.wg.infrastructure.managers.impl
{
    ...
    public class TutorialManager extends TutorialManagerMeta implements ITutorialManager
    {
        ...
        private static const LINKAGE_REG_EXP:RegExp = new RegExp(new RegExp(":[^\\?\\.]*[\\.\\?]","g"));

        private static const CRITERIA_REG_EXP:RegExp = new RegExp(new RegExp("\\?%[^%]*%"));

        private static const PARAMS_REG_EXP:RegExp = new RegExp(new RegExp("\\?.*$"));
        ...
        override protected function showEffect(param1:String, param2:String, param3:TutorialBuilderVO) : void
        {
            var _loc5_:TutorialComponentPathVO = null;
            var _loc6_:DisplayObject = null;
            var _loc4_:IView = this._idToView[param1];
            if(StringUtils.isNotEmpty(param2))
            {
                _loc5_ = this._idToVO[param2];
                if(_loc5_)
                {
                    _loc6_ = _loc5_.foundComponent;
                    if(_loc6_)
                    {
                        this.setupEffectBuilder(_loc4_,param3,_loc6_);
                    }
                }
            }
            else
        ...
        public function as_hideEffect(param1:String, param2:String, param3:String) : void
        {
            var _loc5_:* = 0;
            var _loc4_:TutorialComponentPathVO = this._idToVO[param2];
            if(_loc4_)
            {
                _loc5_ = this._shownIds.indexOf(param2);
                if(_loc5_ != -1)
                {
                    this._shownIds.splice(_loc5_,1);
                }
                this.stopEffects(param1,_loc4_.foundComponent?this._componentToVO[_loc4_.foundComponent].id:param2,param3);
            }
        }
        ...
        public function as_setCriteria(param1:String, param2:String) : void
        {
            ...
            if(this._criteriaHash.hasOwnProperty(param1))
            {
                ...
                if(_loc3_.criteriaValue != param2)
                {
                    ...
                    {
                        this.as_hideEffect(_loc3_.viewTutorialId,_loc3_.vo.id,Values.EMPTY_STR);
        ...
        public function registerUnboundComponent(param1:String, param2:String, param3:DisplayObject) : void
        {
            ...
            var _loc6_:* = this._unboudViewsForRegister[param1] != null;
            if(_loc6_)
            {
        ...
        public function setupEffectBuilder(param1:IView, param2:TutorialBuilderVO, param3:DisplayObject) : void
        {
            ...
            if(param1 != null)
            {
                _loc5_ = param1.as_config.viewTutorialId;
                _loc6_ = param3 != null?this._componentToVO[param3].id:Values.EMPTY_STR;
                _loc4_ = this._buildersMap.getBuilder(_loc5_,_loc6_,param2.builderLnk);
                if(_loc4_ == null)
                {
                    _loc4_ = App.utils.classFactory.getObject(param2.builderLnk) as ITutorialBuilder;
                    _loc4_.setView(param1);
                    if(param3 != null)
                    {
                        _loc4_.component = param3;
                    }
                    this._buildersMap.addBuilder(_loc4_,_loc5_,_loc6_,param2.builderLnk);
                }
                _loc4_.updateData(param2.data);
            }
        }
