package net.wg.gui.battle.views.questProgress.helper
{
    ...
    public class QuestProgressTopViewItemsPos extends Object implements IDisposable
    {
        ...
        public function recalculate(param1:int, param2:int, param3:int) : void
        {
            ...
            if(param3 >= 0)
            {
                _loc5_ = _loc5_ - (HALF_SEPARATOR_FULL_WIDTH >> 1);
            }
            if(param2 > 0)
            {
                ...
                if(param1 > 0)
                {
                    this._flagToX = this._flagToX - ITEMS_GAP;
                }
                _loc5_ = _loc5_ + (ITEMS_STATIC_WIDTH >> 1);
            }
            else
            {
                _loc5_ = _loc5_ - (ITEMS_GAP - SEPARATOR_PADDING + SEPARATOR_WIDTH);
            }
            var _loc6_:* = 0;
            while(_loc6_ < param1)
            {
                ...
                if(param3 >= 0 && _loc6_ == param3)
                {
                    ...
                    _loc5_ = _loc5_ + (SEPARATOR_PADDING + SEPARATOR_WIDTH);
                }
                _loc5_ = _loc5_ + WIDTH_BETWEEN_ITEMS;
                _loc6_++;
            }
            if(param3 >= 0)
            {
