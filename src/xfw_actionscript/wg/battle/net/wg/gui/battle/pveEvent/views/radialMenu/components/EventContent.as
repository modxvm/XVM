package net.wg.gui.battle.pveEvent.views.radialMenu.components
{
    ...
    public class EventContent extends Sprite implements IDisposable
    {
        ...
        public function setRotation(param1:Number) : void
        {
            ...
            this.commandContainer.x = -TITLE_OFFSET_RADIUS * Math.sin(_loc4_);
            this.commandContainer.y = -Math.abs(TITLE_OFFSET_RADIUS * Math.cos(_loc4_));
            if(param1 != 0)
            {
                this.chatContainer.x = MESSAGE_OFFSET * Math.sin(_loc4_);
            }
            this.chatContainer.y = MESSAGE_OFFSET * Math.abs(Math.sin(_loc2_));
        }
    }
}
